...
function Navbar() {
  ...
  return (
    <nav className="flex  fixed sm:w-full md:w-fit max-sm:w-full z-50">
      <div className="md:flex flex-col h-screen border-r-2 border-slate-600/40 w-40 bg-sidebar gap-10 max-sm:hidden sm:hidden">
        <div className="flex">
          <div className="h-16 w-full flex justify-center items-center bg-secondary gap-1">
            <Bug />
            <p className="text-white text-lg font-bold">IssueTracker</p>
          </div>
        </div>
        <div className="p-1 flex flex-col justify-between h-full w-full">
          <ul className="bg-white/10 flex flex-col items-start justify-center gap-x-4 w-full rounded-xs">
            <Link href="/issues" passHref className="w-full">
              <li
                className={`flex items-center p-2 w-full gap-2 rounded-xs cursor-pointer transition-colors hover:bg-muted/80 ${
            ...
            <Link href="/" passHref className="w-full">
              <li
                className={`flex items-center p-2 w-full gap-2 rounded-xs cursor-pointer hover:bg-muted/80 transition-colors ${
            ...
            {status === "authenticated" && (
              <p
                className="w-full `flex items-center p-2 gap-2 rounded-xs cursor-pointer hover:bg-muted/80 transition-colors"
                onClick={() => signOut()}
              >
          ...
          {status === "authenticated" && (
            <DropdownMenu.Root>
              <DropdownMenu.Trigger className="w-full items-center rounded-xs cursor-pointer border-2 border-muted/80 hover:border-blue-400/90 transition-colors ">
                <Avatar
      ...
      <div className=" flex h-16 md:w-fit  justify-between max-sm:w-full sm:w-full bg-sidebar md:bg-transparent items-center ">
        <div className="md:flex border-t-64 border-secondary  border-l-4 border-r-transparent border-r-20 md:h-0 md:w-0 relative right-1 max-sm:hidden sm:hidden">   
        </div>
        <div className="flex items-center justify-center gap-2 pl-3 md:hidden">
          <Bug className="stroke-white h-6 w-6" />
          <p className="text-white text-base font-medium">IssueTracker</p>
        </div>
        <ul className="flex flex-row items-center justify-end gap-x-6 w-auto rounded-xs md:hidden mr-5">
          ...
          {status === "authenticated" && (
            <Button variant="outline" onClick={() => signOut()}>
              <LogOutIcon className="stroke-secondary h-5 w-5" />Sign Out
            </Button>
          )}
          {status === "unauthenticated" && (
            <Button variant="outline" onClick={() => signIn()}>
              <LogIn className="stroke-secondary h-5 w-5" />Sign In
            </Button>
          )}
        </ul>
      </div>
